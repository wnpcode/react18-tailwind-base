...
import { countTax, formatCurrencyText, handleCombo } from "@/configs/utils";
...
const EditProduct = () => {
  ...
  const getData = async () => {
    try {
      ...
      setData({
        ...
        hpp: response.data.buyingPrice,
        tax: response.data.sellingPrice / response.data.tax,
        price: response.data.sellingPrice + response.data.tax,
      });
  ...
  const saveData = async () => {
    ...
    let tax = (formatCurrencyText(_.get(data, "tax")) / 100) * sellingPrice;
    let newData = {
      ...data,
