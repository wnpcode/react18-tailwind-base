...
import { countTax, handleCombo } from "@/configs/utils";
...
const AddProduct = () => {
  ...
  const getData = async () => {
    try {
      ...
      let response = await axiosGet(`api/v1/admin/products/${params.id}`);
  ...
  const getCategory = async () => {
    try {
      ...
      setCategoryList(_.map(response.data.content, handleCombo));
      loading.setLoading(false);
    } catch (error) {
      loading.setLoading(false);
    }
  ...
  const saveData = async () => {
    ...
    try {
      loading.setLoading(true);
      let stock = parseInt(_.get(data, "stock").replaceAll(".", ""));
      let buyingPrice = parseInt(_.get(data, "hpp").replaceAll(".", ""));
      let sellingPrice = parseInt(
        _.get(data, "sellingPrice").replaceAll(".", "")
      );
      let tax = (_.get(data, "tax").replaceAll(".", "") / 100) * sellingPrice;
      let response = await axiosPost(`api/v1/admin/products`, {
        ...data,
      ...
      <Card className="mt-12 ">
        <CardBody className="">
          ...
          <div className="flex flex-row gap-1">
            {_.map(_.toArray("123456"), (item) => {
              return (
                <>
                  <Card
                    className={`group relative flex aspect-square w-1/6 cursor-pointer items-center justify-center overflow-hidden bg-gray-200`}
                    onClick={() => {
            ...
            {/* <Card
              className="flex aspect-square w-1/6 cursor-pointer items-center justify-center overflow-hidden"
              onClick={() => {
