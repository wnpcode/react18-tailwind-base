...
import { getLoginData, handleSort, handleTableData } from "@/configs/utils";
...
const CategoryList = () => {
  ...
  const getMerchantList = async () => {
    ...
    try {
      ...
      let response = await axiosGet(`api/v1/admin/users`, query);
    ...
    // try {
    //   //   let response = await axiosGet("");
    //   //   setMerchantList(response.data.content);
  ...
  const getData = async () => {
    ...
    try {
      loading.setLoading(true);
      let response = await axiosGet(`api/v1/admin/categories`, query);
      loading.setLoading(false);
    ...
    try {
      ...
      if (data.id)
        response = await axiosPut(
          `api/v1/admin/categories/${data.id}`,
          formdata
        );
      else var response = await axiosPost(`api/v1/admin/categories`, formdata);
      setDetailData({});
  ...
  const handleDelete = async () => {
    try {
      ...
      let response = await axiosDelete(
        `api/v1/admin/categories/${deleteData.id}`
      );
      setDeleteData({});
